...
link = []
def page_Urls(response_content, url):
    soup = BeautifulSoup(response_content, "html.parser")
    r = soup.find("div", {"data-hook": "cr-filter-info-review-rating-count"})
    x = int(r.get_text().strip().split(", ")[1].split(" ")[0]) // 10 + 1
    cleaned_url = re.sub(r'&pageNumber=\d+&sortBy=recent', '', url)
    for i in range(x):
        modified_urls = f"{cleaned_url}&pageNumber={i + 1}&sortBy=recent"
        link.append(modified_urls)
        
    url_sessions(link, userAgents)
...
#                             soup = BeautifulSoup(response.content, "html.parser")
#                             next_page = soup.find('li', {'class': 'a-last'})  # Check for the 'Next' button
#                             if next_page and 'a-disabled' not in next_page['class']:  # If 'Next' button is not disabled
#                                 pg += 1  # Increment page number
#                             else:
            ...
            while attempts < max_tries and not response:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        x = int(r.get_text().strip().split(", ")[1].split(" ")[0]) // 10 + 1
                        if x > 1:
                            page_Urls(response.content, url)
                        else:
                            break
                    break
                        # for i in range(x):
                        #     modified_urls = f"{url}&pageNumber={i + 1}&sortBy=recent"
                        #     break
                        # print(reviews)
                    #break
                except RequestException as e:
